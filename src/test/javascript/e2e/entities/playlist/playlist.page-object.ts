...
export class PlaylistUpdatePage {
  ...
  listIdInput = element(by.id('field_listId'));
  ...
  async setListIdInput(listId) {
    await this.listIdInput.sendKeys(listId);
  }

  async getListIdInput() {
    return await this.listIdInput.getAttribute('value');
  }
