...
public interface UserEqualizerSettingRepository extends JpaRepository<UserEqualizerSetting, Long> {
    ...
    @Query("select userEqualizerSetting from UserEqualizerSetting userEqualizerSetting where userEqualizerSetting.user.login = ?#{principal.username}")
    List<UserEqualizerSetting> findByUserIsCurrentUser();
